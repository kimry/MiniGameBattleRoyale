...
$io->on('connection', function ($socket) use ($io) {
    ...
    $socket->on('gameStart',function() use($io,$socket){
        $io->userList["$socket->roomNumber"]["count"]=4;
        $io->userList["$socket->roomNumber"]["dummy"]=0;
    ...
    $socket->on('requestOpScreen', function($opuserid, $game) use($io,$socket){
        if(array_search($opuserid,$io->userList["$socket->roomNumber"]["clearuserid"]))
        {
            echo "$socket->userid can't enter the $opuserid!!\n";
            $socket->emit('backwaitingroom',$game);
        }
        else
        {
            if($opuserid==$io->userList["$socket->roomNumber"]["clearuserid"][0])
            {
                echo "$socket->userid can't enter the $opuserid!!\n";
                $socket->emit('backwaitingroom',$game);
            }
    ...
    $socket->on('opscreenExit',function() use($socket){
        echo "$socket->userid enter waitingroom from $socket->opid\n!!";
    ...
    $socket->on('gameClear',function($game) use($socket, $io){
        ...
        $room = $socket->userid."Screen";
        $io->to($room)->emit('opFinish',$game);
        $dummy = $io->userList["$socket->roomNumber"]["dummy"];
        echo "dummy : $dummy\n";
        if(count($io->userList["$socket->roomNumber"]["clearuserid"])==$io->userList["$socket->roomNumber"]["count"]-1)
        {
            $loser=[];
            ...
            if($game!='ss')
            {
                ...
                if($io->userList["$socket->roomNumber"]["dummy"]>0)
                {
                    $io->userList["$socket->roomNumber"]["dummy"]--;
                }
                $io->userList["$socket->roomNumber"]["count"]--;
            }
            else{
                $socket->leave($socket->roomNumber);
                $socket->join("0");
        ...
        if(count($io->userList["$socket->roomNumber"]["clearuserid"])==count($io->userList["$socket->roomNumber"]["id"]))
        {
    ...
    $socket->on('opFinish',function() use($socket){
        echo "$socket->userid enter waitingroom from $socket->opid\n!!";
        $socket->leave($socket->opid);
    });

    $socket->on('timeout',function($game) use($socket,$io){
        echo "$socket->userid timeout!!!\n";
        $room = $socket->userid."Screen";
        $io->to($room)->emit('opFinish',$game);
        $socket->leave($socket->roomNumber);
        $socket->join("0");
        $io->userList["$socket->roomNumber"]["dummy"]++;
        $sum = count($io->userList["$socket->roomNumber"]["clearuserid"])+$io->userList["$socket->roomNumber"]["dummy"];
        $cnt = $io->userList["$socket->roomNumber"]["count"];
        echo "sum = $sum, count = $cnt\n";
        if(count($io->userList["$socket->roomNumber"]["clearuserid"])+$io->userList["$socket->roomNumber"]["dummy"]==$io->userList["$socket->roomNumber"]["count"])
        {
            $io->userList["$socket->roomNumber"]["id"]=$io->userList["$socket->roomNumber"]["clearuserid"];
            $io->userList["$socket->roomNumber"]["clearuserid"]=[];
            if($game!='ss')
